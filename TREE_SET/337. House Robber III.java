/*
    ...
    one entrance to this area, called the "root." Besides the root, each house has one and only one parent house. After a tour, the smart thief realized that "all houses in this place forms a binary tree". It will automatically contact the police if two directly-linked houses were broken into on the same night.
...
public class Solution {
    public int rob(TreeNode root) {
        ...
        /* Either rub root and 4 grandchildren || just rub two children */
        return Math.max(val + root.val, rob(root.left) + rub(root.right));
    } 
}
...
public class Solution {
    ...
    private int robHelper(TreeNode root, HashMap<TreeNode, Integer> map) {
        ...
        val = Math.max(root.val + val, robHelper(root.left, map) + robHelper
            (root.right, map));
        map.put(root, val);
